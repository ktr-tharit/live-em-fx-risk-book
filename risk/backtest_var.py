...
# backtest_var.py
# ----------------
# Implements a Basel-style traffic-light backtest of the VaR model, in two
# layers:

# LAYER A — Historical constant-notional backtest (usable from day one):
#     Pretend a fixed hypothetical position has been held for the entire
#     history in fx_rates.csv. Roll a VaR estimate forward day by day (using
#     only data available up to that day - no lookahead), compare it against
#     that day's actual hypothetical P&L, and count exceptions over a rolling
#     window. This validates whether the VaR METHODOLOGY is sound, independent
#     of what we actually traded.

# LAYER B — Live exception tracking (builds up as the project runs):
#     Uses the actual position(s) logged in positions.csv from the date they
#     were put on, and tracks real exceptions going forward. This will not have
#     enough days to run a full 250-day Basel test for a while - that's
#     expected and should be stated explicitly, not hidden.

# Exception rule: exception = 1 if actual_pnl < -VaR_estimate (i.e. loss
# worse than the VaR threshold predicted)

# Traffic light zones (based on exceptions in a 250-day window, 99% VaR):
...
# LAYER_A_NOTIONAL_USD = 1_000_000  # hypothetical constant LONG_USD position, single pair for simplicity
# LAYER_A_PAIR = "USDTHB"
...
#     """
#     Historical constant-notional backtest on a single pair (simplest possible
#     version - extend to multi-pair once this is validated and working).
#     """
#     series = returns[LAYER_A_PAIR].dropna()
#     records = []

#     for i in range(VAR_LOOKBACK_DAYS, len(series)):
#         history_window = series.iloc[i - VAR_LOOKBACK_DAYS:i]  # no lookahead
#         var_pct = -np.percentile(history_window, (1 - CONFIDENCE) * 100)
#         var_usd = var_pct * LAYER_A_NOTIONAL_USD

#         actual_return = series.iloc[i]
#         actual_pnl_usd = actual_return * LAYER_A_NOTIONAL_USD
...
#         result["exception"].rolling(BACKTEST_WINDOW_DAYS, min_periods=1).sum()
#     )
...
#     """
#     Live backtest using actual logged positions, starting from each
#     position's as_of_date. Will have few rows early in the project - that's
#     expected, not a bug.
#     """
...
#         pair = pos["pair"]
#         if pair not in returns.columns:
#             continue
#         pair_returns = returns[pair].dropna()
#         pair_returns = pair_returns[pair_returns.index >= pos["as_of_date"]]
#         if pd.notna(pos["end_date"]):
#             pair_returns = pair_returns[pair_returns.index <= pos["end_date"]]

#         for date, ret in pair_returns.items():
#             history = returns[pair].dropna()
#             history = history[history.index < date].tail(VAR_LOOKBACK_DAYS)
#             if len(history) < 30:  # not enough history yet to estimate VaR sensibly
#                 continue
#             var_pct = -np.percentile(history, (1 - CONFIDENCE) * 100)
#             var_usd = var_pct * pos["notional_usd"]

#             actual_pnl_usd = ret * pos["notional_usd"] * pos["direction_sign"]
#             exception = int(actual_pnl_usd < -var_usd)
...
#     print("=== LAYER A: Historical constant-notional backtest ===")
#     layer_a = run_layer_a(returns)
#     layer_a.to_csv(OUTPUT_FILE_A, index=False)
#     if not layer_a.empty:
#         latest = layer_a.iloc[-1]
#         print(f"Latest rolling exceptions ({BACKTEST_WINDOW_DAYS}d window): "
#               f"{int(latest['rolling_exceptions'])} -> {latest['traffic_light_zone']}")
#     print(f"Saved to {OUTPUT_FILE_A}\n")

#     print("=== LAYER B: Live position backtest ===")
#     layer_b = run_layer_b(returns)
#     layer_b.to_csv(OUTPUT_FILE_B, index=False)
#     if layer_b.empty:
#         print("No live exceptions data yet - need a few weeks of positions.csv history.")
#     else:
#         num_exceptions = layer_b["exception"].sum()
#         num_days = len(layer_b)
#         print(f"Live exceptions so far: {num_exceptions} out of {num_days} position-days")
#         print("Note: too few days for a formal 250-day Basel classification yet - "
#               "this is expected at this stage of the project, state it explicitly "
#               "rather than forcing a zone classification on insufficient data.")
